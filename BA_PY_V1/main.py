...
command = "From now on only generate quests if the message explicitly requests you to do so!"
instructions = "You are now a generator of video game quests for a role-playing game. Generate the quest only in the provided JSON structure and only when explicitly requested to do so. Always give the player the option to accept or decline a quest in the dialogue. The NPC offering the quest should react accordingly. The player character is a stranger who arrives in the village and is not identical to any of the NPCs mentioned in the narrative. In the quest, exclusively use NPCs, locations, items, and factions that are known to you from the given narrative. Do not create new locations, NPCs, items, or factions."
...
# structure = "{\"Name\": \"Quest Name\", \"Description\": \"The abstract description of the quest, which only tells the player what the general task is but not how the quest unfolds and what twists await them\", \"By\": \"NPC who assigns the quest\", \"Locations\": [ { \"Name\": \"Location Name\" } ], \"NPCs\": [ { \"Name\": \"NPC Name\" } ], \"Consequences\": [ { \"Action\": \"What the action triggers\" } ] , \"SubStates:\": \"A list of substates that the quest assumes after completing individual subtasks\", \"QuestStatus\": \"The status of the quest, whether active or paused\", \"DialogueOptions\": [ { \"DialogueID\": \"Dialog ID, which identifies the dialogue. Following the pattern d_n, where d stands for dialogue and n represents the unique number.\", \"PreviousDialog\": \"The ID of the previous dialogue that had to be played for this dialogue to be available for selection.\", \"NPC\": \"The NPC delivering this text\", \"Text\": \"The displayed and spoken dialogue\", \"Action\": \"\" } ] }"
data_structure = {
    "Name": "name of quest",
...
def generate_quest(node_id, quest_type):
    dictionary = {
        "id": node_id,
        "type": quest_type,
    }
...
def give_structure():
    messages.append(
        {"role": "system",
         "content": f"Here is a structure describing a quest for a video rpg game: \n{json_structure}"}
    )
...
def give_narrative():
    messages.append(
        {"role": "system",
         "content": f"Here is the narrative of the world our game takes place in: \n{narrative}"}
    )
    #response = openai.ChatCompletion.create(
    #    model=model,
    #    messages=messages,
...
def run_conversation(user_input):
    ...
    messages.append(
        {"role": "user",
         "content": "The player wants a new quest. Here is his request for the new quest: " + user_input}
    )
    functions = [
        {
            "name": "generate_quest",
...
if __name__ == '__main__':
    ...
    give_narrative()
    system_instruction()
    user_input = input("Please tell us what quest you want to play.")
    user_request_response = run_conversation(user_input)
    print(user_request_response.choices[0].message.content)
